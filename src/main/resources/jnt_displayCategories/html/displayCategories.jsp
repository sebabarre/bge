                ...
                <c:choose>
                    <c:when test="${not empty filteredCategories}">
                        <c:forEach items="${filteredCategories}" var="category" varStatus="status">
                            ...
                            <c:url var="categoryLinks" value="${url.mainResource}">
                            <c:param value="${category.key}" name="cat"></c:param>
                          </c:url>
                            <div id="category${category.key}" style="display:inline">
                              ${!status.first ? separator : ''}<a href="${categoryLinks}">${fn:escapeXml(category.value)}</a>
